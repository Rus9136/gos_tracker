...
- Windows-only CryptoAPI-провайдер (Win 7–10, 32/64). Openssl-несовместим.
...
| **A′. Windows golden-client** (фолбэк) | Windows-VM с реальным Tumar + ключ клиента; автоматизировать нативное окно ввода цены (pywinauto/AutoIt), остальное драйвить своим кодом; отдать как headless-RPC | крипто гарантированно корректно; цена — операционная хрупкость (GUI, лицензия `apiKey` до 2026-07-01, гейт версии `1.0.13.2286`) |
...
- **Ближайший надёжный путь — A′ (Windows golden-client):** реальный Tumar в VM
  даёт корректный `sign`; цену вводим в его нативное окно через UI-автоматизацию
  (pywinauto), остальное драйвим своим кодом. Крипто гарантированно валидно.
...
Полная автоподача собирается на A′ уже сейчас (ценой Windows-узла) либо на C
после ответа Gamma.
...
- [ ] **Реализовать path C** (если PoC зелёный/жёлтый): pygost-шифратор цены
      ...
      (Windows golden-client с pywinauto)
- [ ] **Запрос в Gamma** (трек B, параллельно): `GAMMA_INQUIRY_DRAFT.md` →
      `ws_doc.zip`/SDK + неинтерактивная `EncryptOfferPrice`
