...
# поверх приватной сети). Обязателен: без него агент отказывается стартовать
# (он держит расшифрованные .p12/пароль/PIN клиента — принимать задачи от кого
# угодно нельзя). Явный dev-режим без токена — GZ_AGENT_DEV=1.
AGENT_TOKEN = os.environ.get("GZ_AGENT_TOKEN") or None
AGENT_DEV = os.environ.get("GZ_AGENT_DEV") == "1"
