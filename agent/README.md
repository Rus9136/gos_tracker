# Windows submit-agent («робот» автоподачи)

Отдельный деплой на **Windows-узле** с реальным Tumar CSP + NCALayer. Получает от
...
Зачем он нужен и как организовать сам Windows-узел — см.
`../TENDER_AUTOSUBMIT_PLAN.md` (приложение «организация Windows-узла») и правило
#19 в `../CLAUDE.md`. Здесь — только про сам агент.
...
> Места с пометкой `TODO(recon)` — UI-селекторы визарда, заголовок окна Tumar и
> нюансы NCALayer-логина — заполняются по живому конкурсу (см. «Recon» ниже).

## Установка (на Windows-узле)
