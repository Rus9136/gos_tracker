...
См. README.md для обзора возможностей и команд. Обзорная карта проекта —
ARCHITECTURE.md: фичи и связи, диаграмма конвейера акторов, ER-схема
таблиц, роуты по группам, матрица «что берём / что ещё доступно» по OWS
API. При добавлении актора, таблицы или фичи обновлять её вместе с этим
файлом (здесь — «почему», там — «что где»).
...
- **Сервисы systemd** (юзер `rus`, не `goszakup`/`www-data`):
  ...
  - `goszakup-daily.timer` + `goszakup-daily.service` —
    ...
    `data/.daily.lock` больше не нужен; в live-.service на сервере ещё
    остались `ConditionPathExists`/`ExecStartPre`/`ExecStopPost` вокруг
    lock-файла — лишний, но безвредный артефакт. Шаблон в
    `scripts/systemd/goszakup-daily.*` от него уже очищен — при следующей
    правке юнита накатить шаблон, и расхождение исчезнет.
...
14. **Закрытие `ScrapeRun.finished_at` подстраховано БД-heartbeat'ом.**
    ...
    Две страховки поверх (2026-08-07): (а) синхронные синки (bids/contracts/
    plans) держат прогон в `queue/actors._run_scope` — `finally` закрывает его
    и при падении, иначе `TimeLimitExceeded` (у dramatiq лимит по потоку и
    прилетает исключением в середину работы) оставлял `finished_at=NULL`;
    (б) в начале такого актора — `_skip_duplicate` (`jobs.ingest.
    active_run_of_kind` по префиксу note): если прогон того же вида ещё жив,
    актор выходит. Дубль не «продвигает» работу, а повторяет ту же выборку:
    `bids_synced_at` ставится в конце опроса объявления. 07.08 в очереди
    одновременно висели ШЕСТЬ сообщений `bids_sync_actor` (лишние enqueue шёл
    сам воркер — `daily_actor` при этом не выполнялся), каждое перемалывало те
    же 500 объявлений, а «идёт прогон #N» в UI не гас вовсе. Гейт по префиксу
    (а не мьютекс в Redis) — потому что источник правды о живости прогона уже
    есть в БД и переживает рестарт непёрсистентного redis.
    Плашка «идёт прогон» на `/organization/{id}/report` показывает только
    дозагрузку по БИН ЭТОЙ организации (`own_run`); прочие фоновые синки к
    отчёту отношения не имеют и лишь дизейблят кнопку.
...
16. **Аутентификация — таблица `users`, НЕ env Basic Auth.** Пароли —
    ...
    роли не должна уметь выдавать админ-права). Исключение — явно
    вынесенные в `SYSTEM_PAGES` (сейчас: «Догрузка по БИН» `/ingest`):
    их можно выдать галочкой в роли, но в дефолт «без роли» они НЕ входят.
    `/users`, `/roles`, `/scan` и прочее управление в `SYSTEM_PAGES` не
    выносить. Роль, назначенную
...
20. **Отказ LLM молчалив — его ловит только активный сторож.** Правило #7
    ...
    `telegram_chat_id`, дедуп — Redis-ключ `goszakup:health:alerted:<код>` с
    TTL (`GZ_HEALTH_ALERT_COOLDOWN`), иначе за две недели прилетело бы 300+
    сообщений. Ключ **на каждый вид проблемы отдельно** (`Problem.code`;
    класс — подтип `str`, чтобы текст с плавающими числами вида «94ч назад»
    не попал в ключ и не обнулял cooldown ежечасно): 11.08.2026 общий ключ
    занял алерт про `llm-down`, и всплывшая следом независимая `match-stale`
    попала под чужой cooldown и не ушла вовсе. Дедуп ставится ПОСЛЕ поиска
    получателей — иначе ключ сгорал бы на проблему, о которой никому не
    сообщили. Redis недоступен → шлём (лучше лишнее, чем тишина).
...
22. **Заявки поставщиков с ценами — sealed-bid, видны ТОЛЬКО после дедлайна.**
    `TrdApp`→`AppLots` даёт цену каждого участника (`price`/`amount`/
    `discountPrice`), статус (`Подано`/`Допущено`/`Победитель`/`Второй
    победитель`/`Отклонено`) и БИН — таблица `lot_bids`, синк `jobs/bids.py`,
    актор `bids_sync_actor` (очередь `goszakup_daily`, шлётся из `daily_actor`).
    Замерено: у лотов с ОТКРЫТЫМ приёмом API не отдаёт ни одной заявки, через
    сутки после дедлайна — у 2 объявлений из 10, через 3-10 дней — у 9 из 10.
    Поэтому: (а) это ретроспектива цен и конкурентов, а НЕ способ подсмотреть
    ставки во время гонки — не обещать этого в UI; (б) окно по `dateApply`,
    как у contracts-sync, здесь ДАЁТ НЕВЕРНЫЙ РЕЗУЛЬТАТ: заявка становится
    видна на дедлайне, когда её `dateApply` уже позади пройденного окна, —
    теряются ровно конкурентные лоты. Идём от своих объявлений с прошедшим
    дедлайном. `TrdApp.buyId` не принимает массив (в отличие от `TrdBuy.id`)
    — запрос на объявление. Отбор (`announcements_to_sync`): дедлайн старше
    `GRACE_HOURS`=48 и моложе `RETRY_HORIZON_DAYS`=45, победителя ещё нет,
    `bids_synced_at` пуст или старше `RECHECK_HOURS`=72. **Отметка
    `Announcement.bids_synced_at` обязательна и ставится даже при пустом
    ответе** — иначе объявления без итогов («не состоялась») навсегда заняли
    бы верх выборки: строк в `lot_bids` у них нет, факт опроса восстановить
    неоткуда, и хвост (на старте фичи 2745 объявлений старше недели) не
    разбирался бы никогда. Победитель из заявки не затирает добытое
    HTML-ом/договором и появляется РАНЬШЕ договора (на замере: у 12 лотов из
    40 победитель был в API, когда `lot.winner_bin` пустовал — правило #5).
    Ручной бэкофилл: `cli bids-sync --sync --limit N [--horizon D]`.
    **Число участников — денормализовано в `Lot.bids_count`** (`recount_bids`
    после каждого опроса объявления, включая пустой ответ): считать GROUP BY
    по `lot_bids` на каждый показ списка дорого, а анализ конкуренции — весь
    смысл этих данных. NULL и 0 РАЗНЫЕ и смешивать их нельзя: NULL —
    «объявление не опрашивали», 0 — «опросили, никто не подался» (это и есть
    нулевая конкуренция; на 07.08.2026 таких лотов 2952 из 9928 опрошенных,
    ещё 3259 — с единственным участником). Восстановить факт опроса из пустой
    `lot_bids` неоткуда — отсюда явная запись нуля. В UI показывает макрос
    `rivals_tag` (карточка лота, списки `/past` и `/starred` — на `/actual`
    колонки нет, там заявки ещё закрыты) и колонка «Ср. конкурентов» на
    `/suppliers` (среднее по ВЫИГРАННЫМ лотам: побед в БД больше, чем заявок).
    Бэкофилл — `scripts/backfill_bids_count.py` (без обращений к API).
    Миграции: `2ac010466a64` (таблица), `a1d47569a180` (отметка),
    `bb09e3705f64` (`lots.bids_count`).

23. **Отчёт по поставщикам `/suppliers` — лидогенерация, три источника с
    разной полнотой.** Цель — «кто выигрывает/проигрывает по нашим кодам
    ЕНС ТРУ» + контакты, чтобы предлагать платформу. Победы — из
    `lots.winner_bin` (самый полный источник), суммы — договор с фолбэком
    на плановую сумму лота, вторые места/участия — из `lot_bids` (есть
    только у объявлений, опрошенных bids-sync после дедлайна — «0 заявок»
    значит «нет данных», НЕ «не участвовал»; так и написано в футере
    страницы). Контакты (email/телефон/сайт/адрес) — реестр участников OWS
    `Subjects`: ЮЛ ищутся по `bin`, ИП — ТОЛЬКО по `iin` (12-значный
    идентификатор ИП — это ИИН, фильтр bin его не находит; пробуем оба),
    покрытие неполное — часть ИП в реестре нет. Синк —
    `jobs/supplier_contacts.py` / `cli supplier-contacts-sync` (ad-hoc, без
    очереди, до 2 запросов на организацию при ~1 rps); отметка
    `Organization.contacts_synced_at` ставится и при пустом ответе (паттерн
    `bids_synced_at` из правила #22). Непустые поля реестра перекрывают
    наши, пустые не затирают. Победители, добытые HTML-ом, строки в
    `organizations` не имеют — `orgs_to_sync` их создаёт. Сам отчёт
...
24. **Пайплайн хранит ВСЕ лоты (SaaS-пивот, фаза A) — дорогие стадии только
    ...
    **Фаза B (2026-07-29) — вертикали в UI:** фильтр списков/форм ходит
    параметром `category` (старый `?it=` удалён), псевдо-слаг `other` =
    `category IS NULL` — только в UI-фильтрах и на дашборде, в scope
    пользователя его НЕ пускает `_parse_scope_form` (валидация по
    `CATEGORY_SLUGS`). Палитра чипов — `Vertical.hue`/`VERTICAL_HUES`
    (jinja-глобал), макрос `cat_tag` принимает СЛАГ, не лейбл. IT-only
    фильтры анализа (dev_category, vendor_lock_risk) в `lots.html`
...
- `jobs/bids.py` — синк заявок поставщиков с ценами (правило #22).
  `announcements_to_sync` — отбор объявлений к опросу (grace/recheck/горизонт,
  «победителя ещё нет»); `sync_announcement_bids` — один опрос, ставит
  `bids_synced_at` даже при пустом ответе; `recount_bids` — `Lot.bids_count`
  по всем лотам объявления (ноль ставится явно); `apply_winner_from_bid` —
...
- `jobs/supplier_report.py` — отчёт «кто выигрывает/проигрывает» по кодам
  ЕНС ТРУ + CSV-экспорт (правило #23). Страница `/suppliers` (admin).
- `jobs/supplier_card.py` — карточка одного поставщика (`/supplier/{bin}`,
  admin): победы с суммой договора (фолбэк — план), проигранные заявки со
  своей и победившей ценой, разбивка по заказчикам и ЕНС ТРУ. Ключ — БИН, а
  не `organizations.id`: у победителя из HTML-таба строки в organizations
  может не быть. `/organization/{id}` для поставщика не подходит — там лоты
  по `customer_id`, у поставщика их нет, и страница выходила пустой (отсюда
  же ссылки с `/suppliers` и с карточки лота). Агрегаты — по всем лотам,
  таблицы обрезаны до `ROWS_LIMIT`. Win-rate считается ТОЛЬКО по лотам с
  известной заявкой (правило #22): победы видны почти всегда, участия —
  лишь по опрошенным после дедлайна объявлениям, иначе он был бы всегда
  100%.
