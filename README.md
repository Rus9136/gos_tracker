...
- **Годовой план закупок** — что заказчики только собираются купить:
  вкладка «План закупок» с фильтрами (вертикаль, регион, месяц, способ,
  сумма) и выгрузкой в CSV, врезка «Пункт годового плана» на карточке лота
  (аванс, срок поставки, источник финансирования, за сколько дней до
  объявления закупка появилась в плане) и секция «Годовой план» в отчёте
  организации и вкладка «План закупок» на карточке заказчика.
  У открытых конкурсов пункт публикуется в среднем за 35 дней
  до объявления. По желанию (отдельный тумблер в «Настройках») новые пункты
  плана, подходящие под пре-фильтр запроса, приходят в Telegram.
...
# Годовой план закупок: инкремент (штатно идёт из daily)
.venv/bin/python -m goszakup.cli plans-sync --sync
# Стартовый залив года — один раз, ~2.6 млн пунктов, ~7 часов
.venv/bin/python -m goszakup.cli plans-sync --full --sync
...
| `/actual` | Актуальные тендеры с фильтрами (регион, вертикаль вкл. «Прочее», диапазон суммы; для IT — **тип разработки** и **риск заточки**) |
| `/past` | Прошедшие тендеры (Состоялась/Не состоялась/Отменён/Отказ) с теми же фильтрами |
| `/plans` | **План закупок**: пункты годового плана, по которым объявления ещё нет — фильтры (вертикаль, регион, месяц, способ, сумма), CSV-выгрузка |
...
| `/organization/{id}` | Все лоты конкретной организации + вкладка «План закупок» — что этот заказчик собирается купить в текущем году (+ отчёт по закупкам, admin) |
